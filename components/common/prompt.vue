<template>
	<view>
		...
		<view class="popupContentBox" v-if="popupShow">
			...
			<view class="" v-for="(item, index) of popupConfig.Config" :key="index">
				<view class="popupContent">
					<view class="introduce">{{ item.tips}}</view>
					<input class="input" :type="item.inputType" adjust-position="true" :password="item.password" v-model="item.value"
					 :placeholder="item.placeholder" :maxlength="item.maxlength" focus="true" placeholder-style="color:#999"
					 :confirm-type="item.confirmType" />
				</view>
			</view>
			<view class="popupBut">
				<button @click="onConfirm()">{{ popupConfig.but_title}}</button>
			</view>
		</view>
	</view>
</template>
